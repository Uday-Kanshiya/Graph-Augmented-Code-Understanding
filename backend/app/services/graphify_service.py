    ...
    def run_or_fallback(self, repo_id: str, repo_root: Path, codegraph: GraphDocument) -> GraphDocument:
        ...
        if cli_success:
            try:
                ...
                if hasattr(self.storage, "append_log"):
                    self.storage.append_log(repo_id, "graphify", "warning", f"Failed to parse graph.json generated by CLI: {e}. Falling back to CodeGraph mapping.")
                    
        # 2. Fallback: Build Native Graphify directly from CodeGraph in Python.
        # This completely eliminates the external graphify CLI subprocess dependency.
        
        # Map child nodes to parent nodes based on 'contains' edges in CodeGraph
